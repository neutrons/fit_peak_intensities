...
NUMRUNS=1 #total number of runs
...
do
    echo "running job with python doICCFit.py -r $i $(($i+1)) -p $MANTIDPYPATH"
    nohup python doICCFit.py -r $i $(($i+1)) -p $MANTIDPYPATH #>/dev/null 2>&1  & 
done
